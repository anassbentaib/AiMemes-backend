...
export const signup = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ email: result.email, id: result._id }, 'hallo', { expiresIn: '1h' });
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!isPassword) return res.status(400).json({ message: 'Password not match' });

    const token = jwt.sign({ email: findUser.email, id: findUser._id }, 'hallo');
